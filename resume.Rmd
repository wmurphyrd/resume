...
```{r texts, results='asis', fig.height=.75, fig.width=7.5, fig.align='center'}
...
```{r clouds, fig.height=3, fig.width=3, warning=FALSE, results='asis', fig.pos='right'}

dat %>% filter(type == "wordcloud") %>%
  split(., .$group) %>%
  lapply(function(x) {
    if(nrow(x) < 1) return()
    cat("## ", as.character(x$group[1]), "\n")
    wordcloud(x$title, x$priority, min.freq = 0,
